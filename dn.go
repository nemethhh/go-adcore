...
// This file implements the slice of RFC 4514 (distinguished names) adcore
// needs: parsing, rendering and case-insensitive comparison. It is a
// reimplementation rather than a dependency on github.com/go-ldap/ldap, which
// would pull Kerberos, NTLM, SSPI and BER into a module that performs no I/O —
// and which go-adcore's no-third-party-imports rule forbids outright. The test
// vectors are ported from that project (MIT licence).
...
func ParseDN(str string) (*DN, error) {
	...
	endAttr := func() error {
		if !hasEq {
			return fmt.Errorf("adcore: RDN %q has no '='", string(typeBuf))
		}
		t := strings.TrimSpace(string(typeBuf))
		if t == "" {
			return errors.New("adcore: empty attribute type")
		}
	...
	for i := 0; i < len(str); i++ {
		...
		case escaping:
			if isHexDigit(c) {
				...
				if len(hexBuf) == 2 {
					...
					if err != nil {
						return nil, fmt.Errorf("adcore: bad hex escape %q in %q", hexBuf, str)
					}
			...
			if len(hexBuf) == 1 {
				return nil, fmt.Errorf("adcore: incomplete hex escape in %q", str)
			}
			if strings.IndexByte(escapableChars, c) < 0 {
				return nil, fmt.Errorf("adcore: %q is not an escapable character in %q", c, str)
			}
			appendVal(c, true)
		...
		case c == '#' && len(valBuf) == 0:
			return nil, fmt.Errorf("adcore: hex-encoded BER value in %q is not supported", str)
		default:
			appendVal(c, false)
			leading = false
		}
	...
	if escaping || len(hexBuf) > 0 {
		return nil, fmt.Errorf("adcore: dangling escape in %q", str)
	}
	if err := endRDN(); err != nil {
		return nil, err
	}
...
// EqualFoldDN is the string convenience used wherever a spec value meets a
// value AD echoed back. It is not named EqualFold because at a call site that
// reads as a string comparison, and comparing two DNs as strings is wrong.
...
// SplitDN returns the DN's components, most specific first, each in its
// escaped RFC 4514 form. It parses rather than splitting on commas: an escaped
// comma is part of a component, not a separator, so a lexical split turns
// "OU=Sales\, EMEA,DC=corp" into three components and reparents the object.
func SplitDN(dn string) ([]string, error) {
	parsed, err := ParseDN(dn)
	if err != nil {
		return nil, err
	}
	out := make([]string, 0, len(parsed.RDNs))
	for i := range parsed.RDNs {
		out = append(out, (&DN{RDNs: parsed.RDNs[i : i+1]}).String())
	}
	return out, nil
}

// ContainerOf returns the parent DN. It is derived rather than read back,
// because no directory read returns the parent as its own attribute.
func ContainerOf(dn string) (string, error) {
	...
	p := parsed.Parent()
	if p == nil {
		return "", fmt.Errorf("adcore: %q has no parent", dn)
	}
	return p.String(), nil
}
