...
// that is not KindTransient risks something worse than wasted time:
// KindTransient is the only Kind whose contract guarantees the operation
// provably did not execute (see its doc), so it is the only Kind a backend may
// safely re-issue. A Kind added later must not be folded into this check
// unless it can make that same guarantee.
...
// ClassifyCode maps a Win32 error code (MS-ERREF) to a Kind. It reports false
// for an unmapped code, which the caller must treat as KindUnknown: an
// unrecognized condition is never retried.
//
// Both backends share this table. The PowerShell backend reads the code off
// the exception the ActiveDirectory module raises; the LDAP backend parses it
// out of the diagnostic message, which AD prefixes with exactly these codes in
// hex (for example "0000208D: NameErr:"). Sharing the table is what makes the
// two produce identical Kinds from identical conditions.
func ClassifyCode(code int) (Kind, bool) {
	k, ok := classByCode[code]
	return k, ok
}
